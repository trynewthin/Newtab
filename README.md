...
- 扩展清单版本：`0.4.0`（`manifest.json`）
- 包管理与脚本：`bun`

## v0.4.0 重点更新

- 外观与持久化架构收口：全局外观偏好迁移到独立 `preferences` 域，平台层不再硬编码业务 store rehydrate。
- 启动台布局运行时拆分：`AppGrid` 拆出布局引擎、交互状态机与弹层壳，显著降低热点文件耦合。
- 移除未成熟的玻璃折射材质。
- 去掉新标签页搜索框和图标网格的首屏入场动画。
