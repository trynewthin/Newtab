...
export function LiveActivityArea({ children }: LiveActivityAreaProps) {
    ...
    const reportActivity = useCallback((id: string, isActive: boolean) => {
        setActiveIds(prev => {
            ...
            // Optimization: check if set actually changed
    ...
    return (
        <LiveActivityContext.Provider value={{ reportActivity }}>
            <div
                className={cn(
                    "w-full max-w-4xl transition-all duration-500 ease-in-out overflow-hidden flex flex-col items-center",
                    hasActivities ? "h-auto opacity-100 py-0" : "h-0 opacity-0 pointer-events-none py-0"
                )}
            >
